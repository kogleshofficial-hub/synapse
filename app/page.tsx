...
export default function Home(){
 const [today,setToday]=useState("");const [tasks,setTasks]=useState<Task[]>([]);const [capacity,setCapacity]=useState(120);const [view,setView]=useState<View>("today");const [focus,setFocus]=useState<string|null>(null);const [notice,setNotice]=useState("");
 ...
 useEffect(()=>{const t=dateKey(new Date());setToday(t);setDue(addDays(t,2));try{const raw=localStorage.getItem(TASKS);const parsed=raw?JSON.parse(raw):null;setTasks(Array.isArray(parsed)&&parsed.every(validTask)?parsed:demo(t));const c=Number(localStorage.getItem(CAPACITY));if(c>=30&&c<=360)setCapacity(c)}catch{setTasks(demo(t))}},[]);
